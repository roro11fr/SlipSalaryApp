...
class SendAggregatedEmployeeDataView(views.APIView):
    ...
    def post(self, request):
        ...
        latest = (AuditFile.objects
                  .filter(manager=request.user, period=p,
                          file_type=AuditFile.FileType.CSV,
                          status=AuditFile.Status.CREATED)
...
class SendPdfToEmployeesView(views.APIView):
    ...
    def post(self, request):
        ...
        targets = AuditFile.objects.filter(
            employee__manager=request.user,
            period=p,
            file_type=AuditFile.FileType.PDF,
            status=AuditFile.Status.CREATED,
        )
        ...
        for af in targets:
            send_with_attachment(
                af.employee.email, f"Payslip {p}",
                "Attached payslip. Password: your CNP.",
                [af.file_path]
            )
